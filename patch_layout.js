        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4 flex-1 w-full sm:w-auto">
            <div className="flex flex-col gap-1 flex-1 max-w-sm">
              <label className="text-xs font-bold text-gray-500 uppercase">Selecionar Obra:</label>
              <select 
            ...
            <button onClick={adicionarObra} className="mt-5 flex justify-center items-center px-4 py-2 text-sm font-medium text-white bg-brand-green rounded-lg hover:bg-green-700 transition-colors shadow-sm whitespace-nowrap">
              <Plus className="w-4 h-4 mr-1"/> Nova Obra
            </button>
          </div>
          {activeObra && (
             <button onClick={() => deletarObra(activeObra.id)} className="mt-5 sm:mt-0 px-3 py-2 text-sm text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-900/30 transition-colors flex items-center gap-1 whitespace-nowrap">
        ...
        {activeObra && (
           <div className="flex items-center gap-3 bg-blue-50/50 dark:bg-blue-900/10 p-3 rounded-lg border border-blue-100 dark:border-blue-900/30">
             ...
             <input 
                ...
                className="text-xl font-bold text-gray-800 dark:text-gray-100 bg-transparent border-b border-transparent hover:border-gray-300 dark:hover:border-gray-600 focus:border-blue-500 outline-none w-full max-w-2xl px-1 py-1 transition-colors"
